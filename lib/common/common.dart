...
Widget fadeInImage(String url,double width,double height, {BoxFit fit=BoxFit.cover}) {
  ...
  return CachedNetworkImage(
    imageUrl: url,
    placeholder: (context, url) => Image.asset(AppAssets.placePng, width: width.toDouble(), height: height.toDouble(), fit: fit),

    width: width.toDouble(),
    height: height.toDouble(),
    fit: fit,

    errorWidget: (context, url, _) => Image.asset(AppAssets.placePng, width: width.toDouble(), height: height.toDouble(), fit: fit),
  );
}
...
Widget closeButton(String icon,{Function onTap=backRoute,double? width, Color? icColor}) {
  return GestureDetector(
    ...
    child: Container(
      ...
      child: icon.contains('.svg')?SvgPicture.asset(icon, colorFilter: ColorFilter.mode( icColor ?? grey3A, BlendMode.srcIn), width: width??24):Image.asset(icon, color: icColor ?? grey3A, width: width??24),
    ),
  );
}
...
EdgeInsets padding({double horizontal= 15, double vertical = 0}){
